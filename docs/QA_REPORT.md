...
> ⚠️ **本文档为历史记录。上文 Round 1/2 的用例计数（182/190、196/196、201/201）
> 已被真机复验取代。当前权威结论见 [R3.1 v1.1.2 复验](#r31-v112-复验当前权威计数)：
> 干净解压 **337/337 全绿**（10 个模块）。**
...
### R3.1 v1.1.2 复验（当前权威计数）

**触发原因**：真机使用中出现了一次误判 —— 64 GB 内存的机器上，224 GB（7 个分片）的
397B 模型「测试成功」并给出 28 组数据，多个模型「启动极快」。排查确认该次运行处于
`mock` 模式：模型从未加载、llama-server 从未启动，tps 是按模型名里的参数规模算出的公式值
（`prefill = 9000 / √B`、`decode = 200 / √B`）。但报告内没有任何线索能暴露这一点，
反而有两处自相矛盾的表述在互相抵消。

v1.1.2 修复项与新增的 J 模块断言：

| 问题 | 修复 | J 模块断言 |
|---|---|---|
| 报告不含模型体积，无法判断结果是否可能为真 | 新增「模型体量与内存」区块（体积 / 占内存比例 / 结论） | J11–J15 |
| mock 报告页脚写「数据由本机 llama.cpp 后端实测」，与 mock 警示条矛盾 | 页脚按 runner_mode 分支表述 | J16–J18 |
| 显式选 mock 时警示条却说「未检测到可用的 llama-server」 | 按触发原因分文案；并写明数据来自模型名公式 | J20–J23 |
| mock 模式列出从未执行过的「启动参数（可复现）」 | mock 时显式标注「实际并未执行」 | J22 |
| 无跑前可行性校验，224 GB 模型照跑不误 | real 模式直接跳过并标记 OOM，写明体积算式与替代方案 | J30–J34 |
| 硬件采集受 runner_mode 影响 → Windows 显示「内存大小：未知」 | 采集与 runner_mode 解耦；通用采集器补 Windows ctypes 取内存 | J28–J29 |
| 引擎写入 `status="failed"` 而枚举里没有该值 → API 回传 422 | `ModelStatus` 补 `failed`，并加赋值后仍需通过校验的断言 | J34b, J37 |
...
D. 报告正确性                      35/35   OK
E. 前端逻辑                       32/32   OK
F. API 契约与鲁棒性                 54/54   OK
G. 工程卫生                       26/26   OK
H. 曲线悬停数值标签                   26/26   OK
I. 零基础用户友好度                   48/48   OK
J. 体量可行性与 mock 免责声明           45/45   OK
合计: 337/337 通过，0 失败
QA VERDICT: 全部通过
```

**仍未验证**（受环境限制）：真实浏览器视觉渲染、完整 49 档长压测、大 ctx 的显存 OOM 边界、
全新无 Python 环境下的 `start.bat` 安装分支、以及**真实加载 224 GB 模型的失败报错原文**
（会读爆内存/磁盘，刻意不做；改为在跑前用体积算式拦截，并由 J30–J34 保证不发起加载）。

---

## R3. v1.1.1 实测复验（历史结论，计数已被 R3.1 取代）

> **本节是 v1.1.1 发布前在 Windows 11 真机上重新跑出的结果，为当前唯一权威计数。**
> 上文 Round 1 的 `182/190` 与 Round 2 的 `196/196`、`201/201` 是 v1.1.0 开发期的
> **历史记录**（当时部分模块因缺少 `reports/` 产物会整块中断，计数被截断），
> 保留原文以溯源，但**请勿再引用**。

**环境**：Windows 11 · Python 3.12.10 · llama.cpp build 10989（Vulkan）·
AMD 集成显卡（统一内存架构）· 模型 Qwen3.5-0.8B-Q8_0
...
（v1.1.0 同一环境实测为 **141/146**，5 项失败 —— 差异来自本节所述修复。）

**v1.1.0 在真机上暴露、v1.1.1 已修复的缺陷**：

| # | 级别 | 缺陷 | 修复位置 |
|---|---|---|---|
| 1 | P0（数据正确性） | `warmup_runs=1` 时正式测量请求命中 KV cache，`timings.prompt_n` 从 251 塌到 4，prefill 吞吐被低估约 50 倍并静默写进报告 | `runners/real_runner.py` payload 加 `cache_prompt:false`；`metrics.py` 加异常检测 |
| 2 | P0（完全不可用） | `start.bat` 用 `pythonw.exe` 启动，`sys.stdout=None` 导致 uvicorn 日志初始化抛 `AttributeError`，后端从未监听 | `run.py` 新增 `_ensure_std_streams()` + `reports/startup-error.log` |
| 3 | P1 | Windows 下报告写成 CRLF，Node harness 正则 `;\n` 匹配失败，E/H 模块整块崩 | `report/builder.py` 三处加 `newline="\n"`；harness 正则改 `/;\r?\n/` |
| 4 | P1（测试不自洽） | D/E/H/I 硬依赖开发机 `reports/` 残留（发布包内不存在），D 硬编码 2×49；F 用未排空的 `subprocess.PIPE` 把后端写死、用 POSIX `pgrep`/`/bin/echo` | 新增 `tests/_fixture.py` 确定性夹具；F 改文件重定向 + 跨平台进程查询 |
| 5 | P1 | `RealRunner` 的 httpx 未关 `trust_env`，本机设 `HTTP_PROXY` 时连 127.0.0.1 也走代理 | 三处 `httpx.Client(..., trust_env=False)` |
| 6 | P2 | `stop.bat` 缺 `enabledelayedexpansion`（提示分支死代码）；`start.bat` 用 `timeout` 导致就绪等待在非控制台空转 | `stop.bat:3`、`start.bat` 改 `ping -n 2 127.0.0.1` |

**v1.1.1 端到端真机复验**（真实 llama-server，`runner_mode=real`，未降级 mock）：

- 删除 `config.json` 后启动 → 引擎与模型路径**自动回填**到内置 `llama.cpp/` 与 `models/`（零配置）
- `runner-mode` 的 `effective == real`（未静默降级）
- 4 个输入档位（250/500/1000/2000）实测 `input_tokens` 与档位一致（修复前恒为 4），
  `prefill_tps` 为真实量级（修复前约 31 tps）
- 报告落盘且为 **LF** 行尾

**仍未验证**（受环境限制，不臆测）：真实浏览器视觉渲染、完整 49 档长压测、
大 ctx 档的显存 OOM 边界、全新无 Python 环境下的 `start.bat` 安装分支。
